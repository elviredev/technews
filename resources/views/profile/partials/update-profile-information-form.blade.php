            ...
            @if ($user instanceof \Illuminate\Contracts\Auth\MustVerifyEmail && ! $user->hasVerifiedEmail())
                <div>
                    <p class="tw-text-sm tw-mt-2 tw-text-gray-800">
                        ...
                        <button form="send-verification" class="tw-underline tw-text-sm tw-text-gray-600 tw-hover:text-gray-900 tw-rounded-md tw-focus:tw-outline-none tw-focus:tw-ring-2 tw-focus:tw-ring-offset-2 tw-focus:tw-ring-indigo-500">
                            {{ __('Click tw-here to re-send the verification email.') }}
                        </button>
                    </p>

                    @if (session('status') === 'verification-link-sent')
                        <p class="tw-mt-2 tw-font-medium tw-text-sm text-green-600">
                            {{ __('A new verification link tw-has been sent to your email address.') }}
                        </p>
                    @endif
                </div>
            @endif
